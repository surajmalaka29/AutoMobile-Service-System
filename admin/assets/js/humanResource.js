...
document.addEventListener('DOMContentLoaded', function() {
  ...
  searchBox.addEventListener('input', function () {
    ...
    fetch(`functions/search_officers.php?customer=${encodeURIComponent(query)}`)
...
function getOfficerInfo(officer_id) {
  fetch(`/AutoMobile Project/admin/functions/get_officer_info.php?officer_id=${officer_id}`)
    .then(response => response.json())
    .then(data => {
      const form = document.getElementById('newTransactionForm');
      ...
      form.officerId.value = officer_id;
...
document.querySelectorAll('.delete').forEach(button => {
  button.addEventListener('click', function (event) {
      ...
      window.location.href = "/AutoMobile Project/admin/functions/addOfficer.php?officer_id=" + officerId;
  });
});
