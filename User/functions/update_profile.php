...
if (isset($_POST['updateBtn'])) {
    ...
    // Validate and handle zip code
    if (empty($zipCode)) {
        $zipCode = "NULL"; // Use NULL for empty zip code
    } else {
        // Validate zip code (example: only digits)
        if (!ctype_digit($zipCode)) {
            $_SESSION['message'] = "Invalid zip code. Please enter only numeric values.";
            header('Location: ../profile.php');
            exit();
        }
