...
function fetchNotifications($user_id) {
    ...
    $sql = "SELECT notify_id, date, description,status,delete_status FROM notification WHERE cus_id = ? AND delete_status != 1 ORDER BY date DESC";
...
function markAllAsRead($user_id) {
    ...
    $sql = "UPDATE notification SET status = 1 WHERE cus_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('i', $user_id);
...
function deleteReadMessages($user_id) {
    ...
    $sql = "UPDATE notification SET delete_status = 1 WHERE cus_id = ? AND status = 1";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('i', $user_id);
